...
const ServiceDetailsPage = () => {
  ...
  const { service, services } = useLoaderData() as {
    service: Service;
    services: Service[];
  };
  ...
  // Localization Helper
  const localizedName =
    !isRtl && service.nameEn ? service.nameEn : service.name;
  const localizedDescription =
    !isRtl && service.descriptionEn
      ? service.descriptionEn
      : service.description;

  const [link, setLink] = useState("");
  const [quantity, setQuantity] = useState(service.quantity || 1000);
  ...
  const features = [
    {
      icon: Shield,
      title: t("serviceDetails:features.secure"),
      desc: t("serviceDetails:features.secureDesc"),
    },
    {
      icon: Zap,
      title: t("serviceDetails:features.fast"),
      desc: t("serviceDetails:features.fastDesc"),
    },
    {
      icon: Headphones,
      title: t("serviceDetails:features.support"),
      desc: t("serviceDetails:features.supportDesc"),
    },
  ];
                ...
                <div className="w-full md:w-64 h-48 md:h-auto rounded-lg overflow-hidden bg-muted shrink-0">
                  <img
                    ...
                    className={`w-full h-full object-cover ${
                      service.isActive === false ? "grayscale" : ""
                    }`}
                  />
                </div>
                    ...
                    <div className="flex items-center justify-between py-4 border-t border-border">
                      <div className="text-start">
                        <p className="text-sm text-muted-foreground">
                          {t("serviceDetails:estimatedDuration")}
                        </p>
            ...
            <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-start">
                  {t("serviceDetails:reviews.title")}
                </h2>
                <button
                ...
                >
                  {showReviewForm
                    ? t("serviceDetails:reviews.cancelReview")
                    : t("serviceDetails:reviews.addReview")}
                </button>
              </div>
              ...
              {showReviewForm && (
                <div className="mb-8 bg-secondary/30 p-4 rounded-lg border border-border text-start">
                  <h3 className="font-semibold mb-4">
                    {t("serviceDetails:reviews.newReviewTitle")}
                  </h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm mb-2">
                        {t("serviceDetails:reviews.ratingLabel")}
                      </label>
                    ...
                    <div>
                      <label className="block text-sm mb-2">
                        {t("serviceDetails:reviews.commentLabel")}
                      </label>
                      <textarea
                        ...
                        rows={3}
                        placeholder={t(
                          "serviceDetails:reviews.commentPlaceholder"
                        )}
                      />
                    </div>
                    <button
                    ...
                    >
                      {submittingReview
                        ? t("serviceDetails:reviews.submitting")
                        : t("serviceDetails:reviews.submit")}
                    </button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                {reviews.length === 0 ? (
                  <p className="text-center text-muted-foreground py-4">
                    {t("serviceDetails:reviews.noReviews")}
                  </p>
                ) : (
                  reviews.map((review) => (
                    <div
                      key={review.id}
                      className="border-b border-border pb-4 last:border-0 text-start"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-semibold">
                          {review.userName ||
                            t("serviceDetails:reviews.anonymous")}
                        </h3>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-primary/10 rounded-xl border border-primary/20 p-6 shadow-sm text-start">
              <h3 className="font-bold text-primary mb-3">
                {t("serviceDetails:importantNotes")}
              </h3>
              <ul className="text-sm space-y-2 text-foreground">
                {(
                  t("serviceDetails:notes", { returnObjects: true }) as string[]
                ).map((note, i) => (
                  <li key={i}>✓ {note}</li>
                ))}
              </ul>
